...
def upgrade() -> None:
    ...
    sa.Enum(
        "accepted", "not_accepted", "cancelled", name="statusaccept"
    ).create(op.get_bind())
    sa.Enum("in_work", "done", "not_completed", name="statusprogress").create(
        op.get_bind()
    )
    op.create_table(
        "control",
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["control_id"], ["control.id"], ondelete="SET NULL"
        ),
...
def downgrade() -> None:
    ...
    sa.Enum("in_work", "done", "not_completed", name="statusprogress").drop(
        op.get_bind()
    )
    sa.Enum("accepted", "not_accepted", "cancelled", name="statusaccept").drop(
        op.get_bind()
    )
    sa.Enum(
        "in_work", "not_completed", "done", "canceled", name="statusidp"
